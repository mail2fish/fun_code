...
func (s *Server) setupRoutes() {
	...
	// 添加静态文件服务（传入配置的 Pyodide 完整路径以覆盖 embed）
	staticHandler, err := handler.NewStaticHandler(s.etagCache, s.config.Pyodide.FullPath)
	if err != nil {
	...
	if s.config.Server.Mode == config.ModeAPIGateway {
		...
		auth.Use(s.handler.AuthMiddleware())
		{
			...
			auth.GET("/programs/:id/histories", gorails.Wrap(s.handler.GetProgramHistoriesHandler, nil))
			auth.GET("/programs", gorails.Wrap(s.handler.ListProgramsHandler, nil))
			...
			auth.GET("/excalidraw/boards/search", gorails.Wrap(s.handler.SearchExcalidrawBoardsHandler, nil))
			// 管理员：列出所有（可按用户过滤）
			auth.GET("/excalidraw/boards/all", s.handler.RequirePermission(handler.PermissionManageAll), gorails.Wrap(s.handler.ListAllExcalidrawBoardsHandler, nil))
			...
			auth.GET("/excalidraw/boards/:id/thumbnail", gorails.Wrap(s.handler.GetExcalidrawThumbHandler, handler.RenderProjectThumbnail))

			// 学生只读访问 Excalidraw（由课程引用授权）
			auth.GET("/student/excalidraw/boards/:id", gorails.Wrap(s.handler.StudentGetExcalidrawBoardHandler, nil))
			auth.GET("/student/excalidraw/boards/:id/thumbnail", gorails.Wrap(s.handler.StudentGetExcalidrawThumbHandler, handler.RenderProjectThumbnail))
