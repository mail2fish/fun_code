...
FunCode 是一款专为少儿编程培训设计的 Scratch 图形化编程自托管服务程序。构建后仅包含一个可执行文件，集成了学员管理系统、Scratch 图形化编程系统及服务端存储。用户下载后，无论使用 Windows、Mac 还是 Linux，只需运行该程序，即可本地部署少儿编程服务系统。学员通过浏览器使用用户名和密码登录后，可访问 Scratch 图形化编程系统，编辑的程序文件会直接保存在服务端。无需专用服务器，可选择部署在互联网上，也可仅在局域网内运行，特别适合小型少儿编程培训机构。
